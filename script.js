document.addEventListener('DOMContentLoaded', function () {
    ...
    // Highlight all text on click
    nameInput.addEventListener('click', function () {
        nameInput.select();
    });

    // Name Processing with auto-capitalization
    nameInput.addEventListener('input', function () {
        const nameParts = nameInput.value.trim().split(' ');

        // Capitalize the first letter of each name part
        const capitalizedNames = nameParts.map(name => {
            return name.charAt(0).toUpperCase() + name.slice(1).toLowerCase();
        });

        // Update the field with the capitalized names
        nameInput.value = capitalizedNames.join(' ');

        if (capitalizedNames.length >= 2) {
            const firstName = capitalizedNames[0];
            const lastName = capitalizedNames[capitalizedNames.length - 1];
